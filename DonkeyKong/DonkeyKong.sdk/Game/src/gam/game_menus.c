...
{
	struct image * win_background;
	struct image * doggo;
	win_background = (struct image *) WIN_BACKGROUND;
	doggo = (struct image *) SLEEP_DOG_0_ADDR;

	DrawImage(frame, win_background, 0, 0);

	DrawNumber(frame, points, 314, 230);  	// score located at (314, 230)

	DrawImage(frame, doggo, 260, 350);		// doggo located at (260, 350)
...
void UpdateWinGame()
{
	...
	if (updatePending)
	{
		...
		{
			doggo = (struct image *) SLEEP_DOG_1_ADDR;
		}
